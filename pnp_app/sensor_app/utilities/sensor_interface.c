...
void SensorInterface_Command_BlinkCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    long interval = (long)json_object_get_number(jsonPayload, "interval");
    ...
    if (commandResult == DIGITALTWIN_COMMAND_OK &&
        Sensor_SerializeBlinkCommandResponse(&response, responsePayload, MAX_MASSAGE_SIZE))
    {
...
void SensorInterface_Command_TurnonCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    else
    {
        ...
        char* errorResponse = "\"Failed to execute turnon command\"";
        DigitalTwinClientHelper_SetCommandResponse(commandResponse, errorResponse, statusCode);
    }
...
void SensorInterface_Command_TurnoffCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    else
    {
        ...
        char* errorResponse = "\"Failed to execute turnoff command\"";
        DigitalTwinClientHelper_SetCommandResponse(commandResponse, errorResponse, statusCode);
    }

}

static const char Sensor_RundiagnosticsActiveResponse[] = "{ \"result\": \"rundiagnostics async command initiated and in progress\" }";
static const char Sensor_RundiagnosticsFailureResponse[] = "{ \"result\": \"rundiagnostics failed to process due to internal error\" }";
static const char Sensor_RundiagnosticsInProgressResponse[] = "{ \"result\": \"rundiagnostics already in progress\" }";
static const char Sensor_RundiagnosticsRunningState[] = "\"'rundiagnostics' command active\"";
static const char Sensor_RundiagnosticsSuccessState[] = "\"Process 'rundiagnostics' async command successfully\"";
static const char Sensor_RundiagnosticsErrorState[] = "\"Process 'rundiagnostics' failed\"";

DIGITALTWIN_CLIENT_RESULT SensorInterface_Rundiagnostics_UpdateStatus(SENSOR_RUNDIAGNOSTICS_STATUS commandStatus, const char* response, int statusCode)
{
    ...
    else if (commandStatus == SENSOR_RUNDIAGNOSTICS_COMPLETED)
    {
        ...
        DigitalTwinClientHelper_SetUpdateAsyncStatus(&updateComplete, SensorInterface_RundiagnosticsCommand, appState.rundiagnosticsRequestId, response, statusCode);
...
static void SensorInterface_Command_RundiagnosticsCallback(const DIGITALTWIN_CLIENT_COMMAND_REQUEST* commandRequest, DIGITALTWIN_CLIENT_COMMAND_RESPONSE* commandResponse, void* userInterfaceContext)
{
    ...
    else
    {
        ...
        if (DigitalTwinClientHelper_SetCommandResponse(commandResponse, Sensor_RundiagnosticsActiveResponse/*initiated-and-inprogress*/, DIGITALTWIN_ASYNC_STATUS_CODE_PENDING) != 0)
        {
...
static void SensorInterface_Property_NameCallback(const DIGITALTWIN_CLIENT_PROPERTY_UPDATE* dtClientPropertyUpdate, void* userInterfaceContext)
{
    ...
    else
    {
        ...
        LogInfo("SENSOR_INTERFACE: Updating name property with new value <%s> ...", dtClientPropertyUpdate->propertyDesired);
...
static void SensorInterface_Property_BrightnessCallback(const DIGITALTWIN_CLIENT_PROPERTY_UPDATE* dtClientPropertyUpdate, void* userInterfaceContext)
{
    ...
    LogInfo("SENSOR_INTERFACE: brightness data=<%.*s>", (int)dtClientPropertyUpdate->propertyDesiredLen, dtClientPropertyUpdate->propertyDesired);
...
// SensorInterface_ReportProperty_Internal is a helper function to report a Sensor's properties.
// It invokes underlying DigitalTwin API for reporting properties and sets up its callback on completion.
DIGITALTWIN_CLIENT_RESULT SensorInterface_ReportProperty_Internal(DIGITALTWIN_INTERFACE_CLIENT_HANDLE interfaceHandle, const char* propertyName, const char* propertyData)
{
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Property_ReportState()
{
    ...
    if (Sensor_SerializeStateProperty(payloadBuffer, MAX_MASSAGE_SIZE))
    {
        return SensorInterface_ReportProperty_Internal(appState.interfaceClientHandle, SensorInterface_StateProperty, payloadBuffer);
    }
    else
    {
        LogError("SENSOR_INTERFACE: serialize readonly property state failed");
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Property_ReportAll()
{
    ...
    if (((result = SensorInterface_Property_ReportState()) != DIGITALTWIN_CLIENT_OK) )
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Telemetry_SendAll()
{
    ...
    {
        LogInfo("SENSOR_INTERFACE: Queing of all telemetries to be sent has succeeded");
    }
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Telemetry_SendTemp()
{
    ...
    {
        LogError("SENSOR_INTERFACE: serialize temeletry message temp failed.");
        return DIGITALTWIN_CLIENT_ERROR;
    }
}
...
DIGITALTWIN_CLIENT_RESULT SensorInterface_Telemetry_SendHumid()
{
    ...
    {
        LogError("SENSOR_INTERFACE: serialize temeletry message humid failed.");
        return DIGITALTWIN_CLIENT_ERROR;
    }
}
...
DIGITALTWIN_INTERFACE_CLIENT_HANDLE SensorInterface_Create()
{
    ...
    if ((result = DigitalTwin_InterfaceClient_Create(SensorInterfaceId,  SensorComponentName, SensorInterface_InterfaceRegisteredCallback, (void*)&appState, &interfaceHandle)) != DIGITALTWIN_CLIENT_OK)
    {
        LogError("SENSOR_INTERFACE: Unable to allocate interface client handle for interfaceId=<%s>, componentName=<%s>, error=<%s>", SensorInterfaceId, SensorComponentName, MU_ENUM_TO_STRING(DIGITALTWIN_CLIENT_RESULT, result));
    ...
    {
        LogInfo("SENSOR_INTERFACE: Created DIGITALTWIN_INTERFACE_CLIENT_HANDLE successfully for interfaceId=<%s>, componentName=<%s>, handle=<%p>", SensorInterfaceId, SensorComponentName, interfaceHandle);
