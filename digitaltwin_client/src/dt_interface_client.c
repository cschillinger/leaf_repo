...
DIGITALTWIN_CLIENT_RESULT DT_InterfaceClient_CreateTelemetryMessage(const char* interfaceId, const char* componentName, const char* telemetryName, const char* messageData, IOTHUB_MESSAGE_HANDLE* telemetryMessageHandle)
{
    ...
    if ((interfaceId == NULL) || (componentName == NULL) || (telemetryName == NULL) || (messageData == NULL) || (telemetryMessageHandle == NULL))
    {
        LogError("Invalid parameter(s): interfaceId=%p, componentName=%p, telemetryName=%p, messageData=%p, telemetryMessageHandle=%p",
                 interfaceId, componentName, telemetryName, messageData, telemetryMessageHandle);
        result = DIGITALTWIN_CLIENT_ERROR_INVALID_ARG;
    }
    else
    {
        ...
        else if ((iothubMessageResult = IoTHubMessage_SetProperty(*telemetryMessageHandle, DT_INTERFACE_ID_PROPERTY, interfaceId)) != IOTHUB_MESSAGE_OK)
        {
...
DIGITALTWIN_CLIENT_RESULT DigitalTwin_InterfaceClient_SendTelemetryAsync(DIGITALTWIN_INTERFACE_CLIENT_HANDLE dtInterfaceClientHandle, const char* telemetryName, const char* messageData, DIGITALTWIN_CLIENT_TELEMETRY_CONFIRMATION_CALLBACK telemetryConfirmationCallback, void* userContextCallback)
{
    ...
    else
    {
        ...
        if ((result = DT_InterfaceClient_CreateTelemetryMessage(dtInterfaceClient->interfaceId, dtInterfaceClient->componentName, telemetryName, dataToSend, &telemetryMessageHandle)) != DIGITALTWIN_CLIENT_OK)
        {
    ...
    else
    {
        ...
        else
        {
            ...
            DigitalTwinLogInfo("DigitalTwin Interface : Invoking property callback for interface %s, propertyName=%s, userInterfaceContext=%p", 
                                dtInterfaceClient->componentName, propertyName, dtInterfaceClient->userInterfaceContext);
...
DIGITALTWIN_CLIENT_RESULT DT_InterfaceClient_ProcessTwinCallback(DIGITALTWIN_INTERFACE_CLIENT_HANDLE dtInterfaceClientHandle, bool fullTwin, const unsigned char* payLoad, size_t size)
{
    ...
    else if (dtInterfaceClient->propertyUpdatedCallback == NULL)
    {
        ...
        LogInfo("Interface name <%s> does not have a property update callback registered with it.  Skipping twin update processing", dtInterfaceClient->componentName);
        result = DIGITALTWIN_CLIENT_OK;
    }
    else
    {
...
DIGITALTWIN_CLIENT_RESULT DigitalTwin_InterfaceClient_UpdateAsyncCommandStatusAsync(DIGITALTWIN_INTERFACE_CLIENT_HANDLE dtInterfaceClientHandle, const DIGITALTWIN_CLIENT_ASYNC_COMMAND_UPDATE* dtClientAsyncCommandUpdate)
{
    ...
    else if ((result = DT_InterfaceClient_CreateTelemetryMessage(dtInterfaceClient->interfaceId, dtInterfaceClient->componentName, DT_AsyncResultSchema, STRING_c_str(jsonToSend),  &telemetryMessageHandle)) != DIGITALTWIN_CLIENT_OK)
    {
        LogError("Cannot create send telemetry message, error = %d", result);
        result = DIGITALTWIN_CLIENT_ERROR_OUT_OF_MEMORY;
    }
